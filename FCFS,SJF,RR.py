...
at1,at2,at3,at4,at5,at6,bt1,bt2,bt3,bt4,bt5,bt6,frame,q,AAT,AWT,content,algo = NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE,NONE
...
def UI():
    ...
    global at1,at2,at3,at5,at4,at6,bt1,bt2,bt3,bt5,bt4,bt6,q,AAT,AWT,algo
    ...
    submit.grid(column= 5, row=12, columnspan=2, pady=5, padx=5)
    ...
    ttk.Label(content, text = "Algo:", font = ("Times New Roman", 10)).grid(column = 0, row = 12, padx = 10, pady = 25)
  
    # Combobox creation
    algo = tk.StringVar()
    algoChoosen = ttk.Combobox(content, width = 27, textvariable = algo)
    # Adding combobox drop down list
    algoChoosen['values'] = ('FCFS', 'SJF','RR')
    algoChoosen.grid(column = 3, row = 12)
    algoChoosen.current(0)


def insert():

    global at1,at2,at3,at5,at4,at6,bt1,bt2,bt3,bt5,bt4,bt6,q,algo
...
def findRRWaitingTime(n, bt, wt, quantum): 
    global df
    rem_bt = [0] * n
    for i in range(n): 
        rem_bt[i] = bt[i][1]
...
def findSJFWaitingTime(processes, n, wt): 
    global df
    rt = [0] * n
    for i in range(n): 
        rt[i] = processes[i][1]
    ...
    while (complete != n):
        for j in range(n):
            if ((processes[j][0] <= t) and (rt[j] < minm) and rt[j] > 0):
                minm = rt[j]
                short = j
                check = True
        if (check == False):
            t += 1
            continue
        df = df.append({'Task' : "Process"+str(processes[short][2]), 'Start' : t, 'Finish' : (t+1)}, ignore_index = True)
        rt[short] -= 1
        minm = rt[short] 
        ...
        if (rt[short] == 0): 
            ...
            check = False
            fint = t + 1
            wt[short] = (fint - processes[short][1] - processes[short][0])
  
            if (wt[short] < 0):
                wt[short] = 0
        t += 1

def findFCFSWaitingTime(processes, n, wt):
    global df
    wt[0] = 0
    t = (processes[0][0]+processes[0][1])
    df = df.append({'Task' : "Process"+str(processes[0][2]), 'Start' : (t-processes[0][1]), 'Finish' : t}, ignore_index = True)
    for i in range(1, n):
        if(t <= processes[i][0]):
            wt[i] = 0
            t = (processes[i][0]+processes[i][1])
            df = df.append({'Task' : "Process"+str(processes[i][2]), 'Start' : (t-processes[i][1]), 'Finish' : t}, ignore_index = True)
        else:
            wt[i] = (t - processes[i][0])
            t = t + processes[i][1]
            df = df.append({'Task' : "Process"+str(processes[i][2]), 'Start' : (t-processes[i][1]), 'Finish' : t}, ignore_index = True)
...
def findavgTime(n, bt, quantum): 
    global df, content,AAT, AWT, algo
    wt = [0] * n
    tat = [0] * n
    if(algo.get()=="SJF"):
        findSJFWaitingTime(bt,n,wt)
    elif(algo.get()=="RR"):
        findRRWaitingTime(n, bt, wt, quantum)
    elif(algo.get()=="FCFS"):
        findFCFSWaitingTime(bt,n,wt)

    findTurnAroundTime(n, bt, wt, tat) 
    

    AAT = ttk.Label(content, font=('calibre',10,'normal'),text=""+str(sum(tat) /n)+" Min")
    AWT = ttk.Label(content, font=('calibre',10,'normal'),text=""+str(sum(wt) /n)+" Min")
